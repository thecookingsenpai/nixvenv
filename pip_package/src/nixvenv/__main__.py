...
def main(config_file="./config.json", new=False, delete=False):
    ...
    if delete:
        if not os.path.exists(nvenv_name):
            print(f"[*] Environment does not exist: {nvenv_name}")
        else:
            shutil.rmtree(nvenv_name, ignore_errors=True)
            print(f"[*] Deleted environment: {nvenv_name}")
        os._exit(0)
    # Else we use the environment
    print(f"[*] Reading from environment: {nvenv_name}")
    local_bin_folder = f"{os.getcwd()}/{nvenv_name}/" + config["local_bin"]
    print(f"[*] Local bin folder: {local_bin_folder}")
    load_file = f"{os.getcwd()}/{nvenv_name}/" + config["load_file"]
    new_environment(new)
    load_config()
    load_environment()
...
def load_config():
    ...
    for key in overwrites:
        cmds += f"export {key}={overwrites[key]}" + "\n"
        print(f"[*] Overwriting enviromental variable: {key}")
    # Additions add the value to the environment variable preserving the original value
    additions = config["additions"]
    for key in additions:
        cmds += f"export {key}" + "=${" + key + "}:" + additions[key] + "\n"
        print(f"[*] Adding to enviromental variable: {key}")
    # Setting our environment variables
    print("[*] Setting our environment variables")
    cmds += f"export PATH={local_bin_folder}:" + os.environ["PATH"]
    ...
    print(f"[*] Writing to the load file: {load_file}")
...
def load_environment():
    ...
    if "zsh" in shell:
        cmd = f"source {load_file} && {shell} -f -d"
    else:
        cmd = f"{shell} --rcfile {load_file}"
...
def new_environment(new_flag=False):
    ...
    if os.path.exists(f"{os.getcwd()}/{nvenv_name}"):
        if not new_flag:
            raise Exception(f"[x] Environment already exists: {nvenv_name}")
        print(f"[+] Environment already exists: {nvenv_name}")
        return
    ...
    for cmd in init_cmds_list:
        ...
        if cmd == "here":
            ...
            running_dir = f"{os.getcwd()}/{nvenv_name}"
        elif not os.path.exists(running_dir):
            raise Exception(f"Running directory does not exist: {running_dir}")
        # Compiling the command to be executed before the environment is activated
        single_cmd = init_cmds_list[cmd]
        actual_cmd = f"cd '{running_dir}' && {single_cmd}"
    ...
    for file in files:
        ...
        if not os.path.exists(origin):
            raise Exception(f"[x] Origin file does not exist: {origin}")
        if os.path.exists(destination):
            raise Exception(f"[x] Destination file already exists: {destination}")
        print(f"[*] Copying file: {origin} to: {destination}")
...
if __name__ == '__main__':
    ...
    if params["operation"] in ["activate", "run"]:
